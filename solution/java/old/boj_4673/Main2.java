package boj_4673;
	 ...
	 * 
	 * <<boj_4673>>
	 *  d(n) 메서드를 작성하고, '셀프넘버'만 출력하기 (풀이 2 : d(n)이 셀프넘버가 아닌 점을 이용하여, 배열에 셀프넘버가 아니라는 boolean 값을 각각 저장하고 셀프넘버들만 출력)
...
class Main2 {
	...
	static void printlnSelfNumbers(int limit) {
		boolean isNotSelfNumber[] = new boolean[limit+1]; // 0 무시, 1~10000 인덱스에 셀프넘버가 아니면 true를 저장
		
		for(int i=1; i<isNotSelfNumber.length; i++) {
			if (d(i)<isNotSelfNumber.length) {
				isNotSelfNumber[d(i)] = true; // d(n)의 결과값은 셀프넘버가 아니므로 true로 갱신
			}
		}
		
		for (int i=1; i<isNotSelfNumber.length; i++) { // 1,2,3,...,LIMIT
			if (!isNotSelfNumber[i]) { // 셀프넘버만 출력
	...
	public static void main(String[] args) {
		...
		printlnSelfNumbers(LIMIT); // 닫힌 구간 [1,LIMIT]에 존재하는 셀프 넘버를 출력
	}
}
