...
import java.util.*;
	 ...
	 * 
	 * <<boj_4673>>
	 *  d(n) 메서드를 작성하고, '셀프넘버'만 출력하기 (풀이 3 : d(n)이 셀프넘버가 아닌 점을 이용하여, 셀프넘버가 아닌 값들의 Set에 값을 저장, 1~10000까지 순차적으로 Set에서 조회 후 속하지 않으면 셀프넘버이므로 출력 )
	 ...
	 *  - add(T t) : Set에 T 추가. 같은 객체이면(equals의 반환값이 true이면) 추가 안 됨.
	 *  - contains(T t) : Set에 T가 존재하는지 여부를 boolean으로 반환
	 * 
	 * ===================================================
	 */
...
class Main3 {
	...
	static void printlnSelfNumber(int limit) {
		Set<Integer> notSelfNumberSet = new HashSet<>(); // 셀프넘버가 아닌 값들의 Set
		
		for (int i=1; i<=limit; i++) {
			int notSelfNumber = d(i); // d(i)는 셀프넘버가 아님.
			
			if (notSelfNumber<=limit) { // 셀프넘버가 아닌 값들을 Set에 추가
				notSelfNumberSet.add(notSelfNumber); 
			}
		}
		
		for(int i=1; i<=limit; i++) {
			if (!notSelfNumberSet.contains(i)) System.out.println(i);
		}
	}
