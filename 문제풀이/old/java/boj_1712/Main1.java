...
public class Main1 {
	...
	public static void main(String[] args) throws IOException {
		...
		// 손익분기점 계산
		int brkEvenPoint = getBrkEvenPoint(fixedCost, variableCost, price); // 손익 분기점이 되는 판매액
	...
	// 손익분기점 계산
	private static int getBrkEvenPoint(int fixedCost, int variableCost, int price) {
		int profit = price - variableCost; // 제품 1개를 생산하고 판매했을 때 얻는 판매이익 
		if (profit<=0) return -1; // 판매이익이 0보다 작거나 같으면 손익분기점이 존재하지 않음

		int brkEvenPoint = 0; // 손익분기점의 기댓값
		
		while (!isSurplus(fixedCost, profit, brkEvenPoint)) { 
			brkEvenPoint ++; // 적자일 경우 손익분기점 상향 조정
		}
		
		return brkEvenPoint;
	}
	
	// 총 판매이익(이익 * 판매량)과 고정비용을 비교하여, 흑자인지 여부를 반환
	private static boolean isSurplus(int fixedCost, int profit, int sales) {
		int totalProfit = profit * sales; // 총 판매이익
		return fixedCost < totalProfit; // 고정비용보다 총 판매이익이 커야 흑자임
	}
